...
namespace School_DB_System
{
    ...
    {//DATA MEMBERS
        ...
        public UpdateBus(ViewController viewController, Controller controllerObj,string BusNum) : base(viewController, controllerObj)
        {
            InitializeComponent();
            FillData(BusNum);
        }
        ...
        protected override void Submit_Btn_Click(object sender, EventArgs e)
        {
            try //handles any unexpected error while converting any string to string or query fail
            {
                ...
                int queryRes = controllerObj.UpdateBus(BNum_Txt.Text.ToString(), int.Parse(BCap_Nud.Value.ToString()), BDriver_CBox.SelectedValue.ToString(), Add_Route_Txt.Text.ToString());
